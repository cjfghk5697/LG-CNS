...
# 이중 permutation 및 두 기존 라이더 종류 케이스 전부 확인 후 병합
def try_merging_bundles2(K, dist_mat, all_orders, bundle1, bundle2):
    merged_orders = bundle1.shop_seq + bundle2.shop_seq
    total_volume = get_total_volume(all_orders, merged_orders)
...
# 이중 permutation 및 모든 라이더 종류 케이스 전부 확인 후 병합
def try_merging_bundles3(K, dist_mat, all_orders, all_riders, bundle1, bundle2):
    merged_orders = bundle1.shop_seq + bundle2.shop_seq
...
# 이중 permutation 및 모든 라이더 종류 케이스 전부 확인 후 병합, 결과를 바로 이용한다 가정하고 라이더의 수를 선반영
def try_merging_bundles4(K, dist_mat, all_orders, all_riders, bundle1, bundle2):
    merged_orders = bundle1.shop_seq + bundle2.shop_seq
