...
- 위와 같이 하였을 때 번들을 처음부터 최대 주문 4개까지 묶을 수 있게 했을 때와 비교해서 분명한 성능 차이는 없으면서도 시간이 많이 단축된다. 단, 최적 가중치 탐색 그리드 서치 방식 대신 가중치 조합을 하나로 고정한다면 여기에 드는 시간도 크지 않아서 상관 없을 수 있다.
