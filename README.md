...
## 7/1 제출본 참고사항
1. 기존의 코드를 좀 더 함수화를 하여 간단하게 바꾸었다.
2. weight1과 weight2의 조합별 총비용을 heatmap으로 시각화하는 코드를 제작하고 돌려본 후 가중치 조합 케이스 수를 이전의 14개에서 6개로 줄여서 시간 초과를 피하였다. 단, 시간 초과가 났을 때의 코드에 비해 비용이 올라가긴 했다.
