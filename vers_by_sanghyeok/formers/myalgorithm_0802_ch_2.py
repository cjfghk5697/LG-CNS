...
def simulated_annealing_weights_with_cawr(K, all_orders, all_riders, dist_mat, init_availables, order_comb_possibility, timelimit=60, seed=None, T0=1000, T_mult=2, min_T=1, weight_change_range=0.05, restart_period=50):
    if seed is not None:
        np.random.seed(seed)
        random.seed(seed)
    ...
    all_bundles, _, min_cost = get_init_bundle_4_order_bundle_prefered_with_reassigning_riders(
        K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, order_comb_possibility, weight1, weight2, weight3
    )
    ...
    while epoch>=1 and time.time() - start_time < timelimit:
        ...
        new_bundles, _, new_cost = get_init_bundle_4_order_bundle_prefered_with_reassigning_riders(
            K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, order_comb_possibility, *new_weights
        )
    ...
    final_bundles, result_rider_availables, final_cost = get_init_bundle_4_order_bundle_prefered_with_reassigning_riders(
        K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, order_comb_possibility, *best_weights
    )
...
def parallel_simulated_annealing_with_cawr(K, all_orders, all_riders, dist_mat, init_availables, order_comb_possibility, timelimit=60):
    start_time = time.time()
    ...
    K, all_orders, all_riders, dist_mat,init_availables, order_comb_possibility, timelimit, seeds[i], T0_values[i], T_mult_values[i], min_T_values[i], weight_change_range[i], restart_period_values[i])) 
...
def simulated_annealing(K, all_orders, all_riders, dist_mat, timelimit, all_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, init_availables, order_comb_possibility, is_allow_worse_case):
    #--------------------------------------------- SA init ---------------------------------------------#
...
def algorithm(K, all_orders, all_riders, dist_mat, timelimit=60):
    ...
    min_init_cost_bundles, min_init_cost_rider_availables = parallel_simulated_annealing_with_cawr(K, all_orders, all_riders, dist_mat, init_availables, order_comb_possibility, 15)
    ...
    with Pool(num_core) as pool:
        result = pool.starmap(simulated_annealing, [[K, all_orders, all_riders, dist_mat, timelimit, all_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, init_availables, order_comb_possibility, 1],
                                                    [K, all_orders, all_riders, dist_mat, timelimit, all_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, init_availables, order_comb_possibility, 1],
                                                    [K, all_orders, all_riders, dist_mat, timelimit, all_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, init_availables, order_comb_possibility, 1],
                                                    [K, all_orders, all_riders, dist_mat, timelimit, all_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, init_availables, order_comb_possibility, -1],])
