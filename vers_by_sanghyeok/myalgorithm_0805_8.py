# Weight MP - bundling_opt_ver1_5Orders3~5 + bundling_opt_ver1_KruskalRandom_KCasework
...
def bundling_opt_ver1_5Orders(method_num, K, all_orders, all_riders, dist_mat, timelimit, all_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, init_availables, order_comb_possibility):
    inf = float('inf')
...
def bundling_opt_ver1_KruskalRandom_KCasework(K, all_orders, all_riders, dist_mat, timelimit, all_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, init_availables, order_comb_possibility):
    inf = float('inf')
...
def select_weight_opt_method(method_num, K, all_orders, all_riders, dist_mat, timelimit, all_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, is_allow_worse_case, init_availables, order_comb_possibility):
    if method_num <= 3:
        min_init_cost_bundles, result_rider_availables = bundling_opt_ver1_5Orders(method_num, K, all_orders, all_riders, dist_mat, timelimit, all_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, init_availables, order_comb_possibility)
    elif method_num == 4:
        min_init_cost_bundles, result_rider_availables = bundling_opt_ver1_KruskalRandom_KCasework(K, all_orders, all_riders, dist_mat, timelimit, all_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, init_availables, order_comb_possibility)

    for rider_i in range(3):
        all_riders[rider_i].available_number = result_rider_availables[rider_i]

    result_bundles = simulated_annealing(K, all_orders, all_riders, dist_mat, timelimit, min_init_cost_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, is_allow_worse_case, init_availables, order_comb_possibility)
...
def algorithm(K, all_orders, all_riders, dist_mat, timelimit=60):
    ...
    # ----------------- Weight Search MP and SA combined -------------------- #

    freeze_support()

    num_core = 4
    with Pool(num_core) as pool:
        result = pool.starmap(select_weight_opt_method, [[1, K, all_orders, all_riders, dist_mat, timelimit, all_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, 1, init_availables, order_comb_possibility],
                                                        [2, K, all_orders, all_riders, dist_mat, timelimit, all_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, 1, init_availables, order_comb_possibility],
                                                        [3, K, all_orders, all_riders, dist_mat, timelimit, all_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, 1, init_availables, order_comb_possibility],
                                                        [4, K, all_orders, all_riders, dist_mat, timelimit, all_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, -1, init_availables, order_comb_possibility],])
    
     
    cost_result = [sum(bundle.cost for bundle in cur_solution) / K for cur_solution in result]
    best_bundles_idx = np.argmin(cost_result)
    all_bundles = result[best_bundles_idx]
