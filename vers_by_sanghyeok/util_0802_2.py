# https://github.com/cjfghk5697/LG-CNS/blob/siwoo/0801/han_SA_util.py + 모든 순열 번들링 이용
...
def kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, bundle_merging_function, order_count_upper_limit, avg_method, all_bundles, kruskal_edge_to_divide):
    def find(v):
        while v != parent[v]:
    ...
    if kruskal_edge_to_divide >= 2:
        edges = edges[:len(edges) // kruskal_edge_to_divide]
...
def get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, kruskal_edge_to_divide=1):
    for rider_i in range(3):
        ALL_RIDERS[rider_i].available_number = init_availables[rider_i]
    ...
    # 2개 주문 묶음 생성
    all_bundles, _ = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, try_merging_bundles_by_dist, 2, 'two_seq', all_bundles, kruskal_edge_to_divide)

    # 4개 주문 묶음 생성
    all_bundles, _ = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, try_merging_bundles_by_dist, 4, 'avg', all_bundles, kruskal_edge_to_divide)
    ...
    result_bundles, result_availables = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, try_merging_bundles_by_dist, 3, 'two', new_all_bundles, kruskal_edge_to_divide)
...
def get_init_bundle_4_order_bundle_prefered_with_reassigning_riders(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, kruskal_edge_to_divide=1):
    all_bundles, rider_availables, _ = get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, kruskal_edge_to_divide)
    for rider_i in range(3):
...
def SA_try_merging_bundles_all_permutations(K, dist_mat, all_orders, bundle1, bundle2):
    merged_orders = bundle1.shop_seq + bundle2.shop_seq
    # merged_orders_shp = sorted(merged_orders, key = lambda x : all_orders[x].cook_time)
    # merged_orders_dlv = sorted(merged_orders, key = lambda x : all_orders[x].deadline)
...
def insertion(cur_solution, cnt, all_orders, dist_mat, K):
    for _ in range(cnt):
        ...
        tmp_bundle = Bundle(all_orders, nxt_bundle.rider, [node], [node], 
                            all_orders[node].volume, dist_mat[node, node + K])
        new_bundle = SA_try_merging_bundles_all_permutations(K, dist_mat, all_orders, bundle1=tmp_bundle, bundle2=nxt_bundle) 
