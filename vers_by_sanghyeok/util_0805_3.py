...
def make_new_solution(car_rider, K, cur_solution, all_riders, all_orders, dist_mat, T, is_pre_decreased, init_availables, order_comb_possibility, optimized_order_perms):
    ...
    bundle_nums_to_decompose = set(random.randint(0, len(new_solution) - 1) for _ in range(5))
    
    decomposed_bundles = []
    for bundle_num in bundle_nums_to_decompose:
        temp_bundle = new_solution[bundle_num]
        temp_bundle.rider.available_number += 1

        for order_num in temp_bundle.shop_seq:
            order = all_orders[order_num]
            decomposed_bundles.append(Bundle(all_orders, car_rider, [order.id], [order.id], order.volume, dist_mat[order.id, order.id+K]))
            car_rider.available_number -= 1

    new_solution = [v for i, v in enumerate(new_solution) if i not in bundle_nums_to_decompose]
    new_solution.extend(decomposed_bundles)

    new_solution, result_rider_counts = kruskal_bundling(K, dist_mat, all_orders, all_riders,
                     random.choice([0, 0.5, 1, 1.5, 2]),
                     -random.choice([0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5]),
                      random.choice([-3, -2.5, -2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2, 2.5, 3]),
                      try_merging_bundles_by_dist_possibles_only, 4, 'two', new_solution, order_comb_possibility, optimized_order_perms, False)

    for rider_i in range(3):
        all_riders[rider_i].available_number = result_rider_counts[rider_i]
