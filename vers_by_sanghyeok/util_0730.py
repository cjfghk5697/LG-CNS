...
def get_init_bundle(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, bundle_merging_function, default_get_dist_method, order_count_upper_limit=3):
    ...
    all_bundles, rider_abailables = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, bundle_merging_function, order_count_upper_limit, 'two_seq', all_bundles, default_get_dist_method)
...
def get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, default_get_dist_function):
    for rider_i in range(3):
        ALL_RIDERS[rider_i].available_number = init_availables[rider_i]
    ...
    # 2개 주문 묶음 생성
    all_bundles, _ = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, try_merging_bundles_by_dist, 2, 'two_seq', all_bundles, default_get_dist_function)

    # 4개 주문 묶음 생성
    all_bundles, _ = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, try_merging_bundles_by_dist, 4, 'avg', all_bundles, default_get_dist_function)
    ...
    result_bundles, result_availables = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, try_merging_bundles_by_dist, 3, 'two', new_all_bundles, default_get_dist_function)
...
def get_init_bundle_4_order_bundle_prefered_with_reassigning_riders(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, defualt_get_dist_function):
    all_bundles, rider_availables, _ = get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, defualt_get_dist_function)
    for rider_i in range(3):
...
def try_merging_many_orders_by_dist(K, dist_mat, all_orders, all_riders, merged_orders):
    total_volume = get_total_volume(all_orders, merged_orders)
...
# 주어진 solution의 경로를 visualize + 준비 시간 및 마감 시간 텍스트 표시
def draw_route_solution3(all_orders, solution=None):
    plt.subplots(figsize=(20, 20))
    node_size = 5

    readytimes = [order.ready_time for order in all_orders]
    ...
    for i in range(len(all_orders)):
        ...
        plt.text(x, y, str(readytimes[i]), fontsize=6)
    ...
    for i in range(len(all_orders)):
        ...
        plt.text(x, y, str(deadlines[i]), fontsize=6)
