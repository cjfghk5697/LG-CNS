# https://github.com/cjfghk5697/LG-CNS/blob/siwoo/0801/han_SA_util.py
...
def kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, bundle_merging_function, order_count_upper_limit, avg_method, all_bundles, order_comb_possibility, optimized_order_perms):
    def find(v):
        while v != parent[v]:
...
def get_init_bundle(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, weight3, bundle_merging_function,  order_comb_possibility, optimized_order_perms, order_count_upper_limit=3):
    ...
    all_bundles, rider_abailables = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, try_merging_bundles_by_dist_possibles_only, order_count_upper_limit, 'two_seq', all_bundles, order_comb_possibility, optimized_order_perms)
...
def get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, weight3, order_comb_possibility, optimized_order_perms):
    for rider_i in range(3):
        ALL_RIDERS[rider_i].available_number = init_availables[rider_i]
    ...
    # 2개 주문 묶음 생성
    all_bundles, _ = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, try_merging_bundles_by_dist_possibles_only, 2, 'two_seq', all_bundles, order_comb_possibility, optimized_order_perms)

    # 4개 주문 묶음 생성
    all_bundles, _ = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, try_merging_bundles_by_dist_possibles_only, 4, 'avg', all_bundles, order_comb_possibility, optimized_order_perms)
    ...
    result_bundles, result_availables = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, try_merging_bundles_by_dist_possibles_only, 3, 'two', new_all_bundles, order_comb_possibility, optimized_order_perms)
...
def get_init_bundle_4_order_bundle_prefered_with_reassigning_riders(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, weight3, order_comb_possibility, optimized_order_perms):
    all_bundles, rider_availables, _ = get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, weight3, order_comb_possibility, optimized_order_perms)
    for rider_i in range(3):
...
def SA_try_merging_bundles(K, dist_mat, all_orders, all_riders, order_comb_possibility, optimized_order_perms, bundle1, bundle2):
    shop_seq1 = bundle1.shop_seq
    shop_seq2 = bundle2.shop_seq
    ...
    merged_orders = shop_seq1 + shop_seq2
    merged_orders_shp = sorted(merged_orders, key = lambda x : all_orders[x].ready_time)
...
def insertion(cur_solution, cnt, all_orders, all_riders, dist_mat, K, order_comb_possibility, optimized_order_perms):
    for _ in range(cnt):
        ...
        tmp_bundle = Bundle(all_orders, nxt_bundle.rider, [node], [node], 
                            all_orders[node].volume, dist_mat[node, node + K])
        new_bundle = SA_try_merging_bundles(K, dist_mat, all_orders, all_riders, order_comb_possibility, optimized_order_perms, bundle1=tmp_bundle, bundle2=nxt_bundle) 
...
def make_new_solution(car_rider, K, cur_solution, all_riders, all_orders, dist_mat, T, is_pre_decreased, init_availables, order_comb_possibility, optimized_order_perms):
    ...
    insertion(new_solution, int(momentum * max(1, int(math.log2(T) * 1.2))), all_orders, all_riders, dist_mat, K, order_comb_possibility, optimized_order_perms)
