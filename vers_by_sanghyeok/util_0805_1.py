...
def kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, bundle_merging_function, order_count_upper_limit, avg_method, all_bundles, order_comb_possibility, optimized_order_perms, is_random_num_added, top_n=-1):
    def find(v):
        while v != parent[v]:
    ...
    for i in range(len(all_bundles)):
        for j in range(i + 1, len(all_bundles)):
            ...
            if len(shop_seq1) + len(shop_seq2) == 2:
...
def get_init_bundle(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, weight3, bundle_merging_function,  order_comb_possibility, optimized_order_perms, is_random_num_added, order_count_upper_limit=3):
    ...
    # for _ in range(3000):
    #     all_bundles, rider_abailables = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, try_merging_bundles_by_dist_possibles_only, order_count_upper_limit, 'two_seq', all_bundles, order_comb_possibility, optimized_order_perms, is_random_num_added, top_n=1)
