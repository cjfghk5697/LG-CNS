# https://github.com/cjfghk5697/LG-CNS/blob/han/Han/based%20siwoo/weight%20update%20function%20copy%202/han_SA_util.py
...
def kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, bundle_merging_function, order_count_upper_limit, avg_method, all_bundles, order_comb_possibility):
    def find(v):
        while v != parent[v]:
    ...
    for i in range(len(all_bundles)):
        for j in range(i + 1, len(all_bundles)):
            ...
            diff_score = (dist1 + dist2)* weight3  + r_diff * weight1 + d_diff * weight1 + start_end_diff * weight2
    ...
    for bundle_num1, bundle_num2, diff_score in edges:
        ...
        new_bundle = bundle_merging_function(K, DIST, ALL_ORDERS, ALL_RIDERS, all_bundles[rbn1], all_bundles[rbn2], order_comb_possibility, order_count_upper_limit)
...
def get_init_bundle(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, weight3, bundle_merging_function, order_count_upper_limit=3):
    ...
    all_bundles, rider_abailables = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, bundle_merging_function, order_count_upper_limit, 'two_seq', all_bundles)
...
def get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, order_comb_possibility, weight1, weight2, weight3):
    for rider_i in range(3):
        ALL_RIDERS[rider_i].available_number = init_availables[rider_i]
    ...
    # 2개 주문 묶음 생성
    all_bundles, _ = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, try_merging_bundles_by_dist_possibles_only, 2, 'two_seq', all_bundles, order_comb_possibility)

    # 4개 주문 묶음 생성
    all_bundles, _ = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, try_merging_bundles_by_dist_possibles_only, 4, 'avg', all_bundles, order_comb_possibility)
    ...
    result_bundles, result_availables = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, try_merging_bundles_by_dist_possibles_only, 3, 'two', new_all_bundles, order_comb_possibility)
...
def get_init_bundle_4_order_bundle_prefered_with_reassigning_riders(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, order_comb_possibility, weight1, weight2, weight3):
    all_bundles, rider_availables, _ = get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, order_comb_possibility, weight1, weight2, weight3)
    for rider_i in range(3):
...
def try_merging_bundles_by_dist_possibles_only(K, dist_mat, all_orders, all_riders, bundle1, bundle2, order_comb_possibility, order_count_upper_limit=3, order_count_lower_limit=1):
