# 8-4_bundling_opt_ver1_5Orders3

from util_0805_20 import *
...
def simulated_annealing(K, all_orders, all_riders, dist_mat, timelimit, all_bundles, start_time, car_rider, ALL_ORDERS, ALL_RIDERS, DIST, is_allow_worse_case, init_availables, order_comb_possibility):
    ...
    max_iter_cnt2 = 400
    best_cost = cur_cost
    best_solution = deepcopy(cur_solution)
    T_max1 = T1
    T_min1 = 10000
    T_max2 = T2
    T_min2 = 10000
    ...
    while True:
        ...
        new_solution, new_cost = make_new_solution(
            car_rider, K, cur_solution, all_riders, all_orders, dist_mat, T2, is_pre_decreased, init_availables, order_comb_possibility)
        ...
        if new_cost < cur_cost:
            ...
            p = math.exp((-(new_cost - cur_cost) * E_multiplier) / (T1 * T_mulitiplier))

            print(int(new_cost), int(cur_cost), int(T1), 
                (-(new_cost - cur_cost) * E_multiplier) / (T1 * T_mulitiplier), p)
            ...
            is_pre_decreased = False
        #T *= delta
        T1 = get_nxt_T_with_cos_annealing(T1, T_min1, T_max1, SA_iter_cnt, max_iter_cnt1)
        T2 = get_nxt_T_with_cos_annealing(T2, T_min2, T_max2, SA_iter_cnt, max_iter_cnt2)
