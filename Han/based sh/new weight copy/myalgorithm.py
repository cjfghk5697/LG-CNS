...
def simulated_annealing_weights_with_cawr(K, all_orders, all_riders, dist_mat,init_availables, timelimit=60, seed=None, T0=1000, T_mult=2, min_T=1, weight_change_range=0.05, restart_period=50):
    ...
    while True:
        ...
        bundles, result_rider_availables, cost = get_init_bundle_4_order_bundle_prefered_with_reassigning_riders(
                K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, 1, weight4, order_comb_possibility, optimized_order_perms)
        ...
        if cost < min_init_cost:
            ...
            min_weight_comb = (weight1, weight2, 1, weight4)
        if K <= 150:

            bundles, result_rider_availables, _ = get_init_bundle(K, all_riders, all_orders, DIST, init_availables, weight1, weight2, 1, weight4, try_merging_bundles_by_dist_possibles_only, order_comb_possibility, optimized_order_perms, False, 4)
        ...
        weight1, weight2, weight3, weight4 = (weight1, weight2, 1, weight4) + np.random.uniform(-weight_change_range, weight_change_range, size=4)
        weight1, weight2, weight3, weight4 = np.clip((weight1, weight2, 1, weight4), -10, 10)  # Clip weights to a reasonable range
        for rider_i in range(3):
...
def parallel_simulated_annealing_with_cawr(K, all_orders, all_riders, dist_mat, init_availables, timelimit=60):
    ...
    weight_change_range =  [0.7, 1.05, 1.4, 1.75]# Reduced range for changing weights
