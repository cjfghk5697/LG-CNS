# https://github.com/cjfghk5697/LG-CNS/blob/siwoo/0801/han_SA_util.py + 주문쌍 확인 + 주문 조합 한 번씩만 확인하기 - 속도 및 비용 개선
...
def kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, weight4, bundle_merging_function, order_count_upper_limit, avg_method, all_bundles, order_comb_possibility, optimized_order_perms):
    def find(v):
        while v != parent[v]:
    ...
    for i in range(len(all_bundles)):
        for j in range(i + 1, len(all_bundles)):
            ...
            diff_score = dist1*weight3 + dist2*weight3 + r_diff * weight1 + d_diff * weight1 + start_end_diff * weight2 + twl_avg * weight4
...
def get_init_bundle(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, weight3, weight4, bundle_merging_function,  order_comb_possibility, optimized_order_perms, is_random_num_added, order_count_upper_limit=3):
    ...
    all_bundles, rider_abailables = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, weight4, try_merging_bundles_by_dist_possibles_only, order_count_upper_limit, 'two_seq', all_bundles, order_comb_possibility, optimized_order_perms)
...
def get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, weight3, weight4, order_comb_possibility, optimized_order_perms):
    for rider_i in range(3):
        ALL_RIDERS[rider_i].available_number = init_availables[rider_i]
    ...
    # 2개 주문 묶음 생성
    all_bundles, _ = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, weight4, try_merging_bundles_by_dist_possibles_only, 2, 'two_seq', all_bundles, order_comb_possibility, optimized_order_perms)

    # 4개 주문 묶음 생성
    all_bundles, _ = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, weight4, try_merging_bundles_by_dist_possibles_only, 4, 'avg', all_bundles, order_comb_possibility, optimized_order_perms)
    ...
    result_bundles, result_availables = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, weight1, weight2, weight3, weight4, try_merging_bundles_by_dist_possibles_only, 3, 'two', new_all_bundles, order_comb_possibility, optimized_order_perms)
...
def get_init_bundle_4_order_bundle_prefered_with_reassigning_riders(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, weight3, weight4, order_comb_possibility, optimized_order_perms):
    all_bundles, rider_availables, _ = get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weight1, weight2, weight3, weight4, order_comb_possibility, optimized_order_perms)
    for rider_i in range(3):
