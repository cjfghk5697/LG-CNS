...
class Firefly:
    def __init__(self, bundles):
        ...
        self.make_bundle_to_list()
        

    def make_bundle_to_list(self):
        self.node_list = []
        for bundle in self.bundles:
            for cur_shop in bundle.shop_seq:
                self.node_list.append(cur_shop)
            for cur_dlv in bundle.dlv_seq:
                self.node_list.append(cur_dlv)
    
    def update_bundle(self, bundles):
        self.bundles = bundles[:]
        self.make_bundle_to_list()


def get_hemming_dist(fierfly1, firefly2):
    dist = 0
    for node1, node2 in zip(fierfly1.node_list, firefly2.node_list):
        if node1 != node2:
            dist += 1
    return dist


def FA_try_merging_bundles(cur_rider_cnt, K, dist_mat, all_orders, bundle1, bundle2):
    merged_orders = bundle1.shop_seq + bundle2.shop_seq
    total_volume = get_total_volume(all_orders, merged_orders)
...
def insertion(cur_firefly, cnt, cur_rider_cnt, all_orders, dist_mat, K):
    org_bundles = cur_firefly.bundles
    for _ in range(cnt):
        ...
        new_bundle = FA_try_merging_bundles(cur_rider_cnt, K, dist_mat, all_orders, bundle1=tmp_bundle, bundle2=nxt_bundle) 

        if new_bundle is not None:
            if len(cur_bundle.shop_seq) == 1:
                org_bundles.remove(cur_bundle)
                cur_rider_cnt[cur_bundle.rider.type] += 1
            ...
            cur_rider_cnt[nxt_bundle.rider.type] += 1
            cur_rider_cnt[new_bundle.rider.type] -= 1
            org_bundles.remove(nxt_bundle)
            org_bundles.append(new_bundle)
            
    #cur_firefly.update_bundle(org_bundles)


def FA_get_cheaper_available_riders(cur_rider_cnt, all_riders, rider):
    for r in all_riders:
...
def mutation(cur_firefly, cnt, cur_rider_cnt, all_orders, all_riders):
    for _ in range(cnt):
        cur_bundle = random.choice(cur_firefly.bundles)
