...
def get_minkowski_dist_by_coords(x1, y1, x2, y2, p=1.6):
...
def kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, order_count_upper_limit, avg_method, all_bundles, bundle_merging_function, default_get_dist_function, weights):
    def find(v):
        while v != parent[v]:
    ...
    for i in range(len(all_bundles)):
        for j in range(i + 1, len(all_bundles)):
            ...
            priority_tier = dist1 + dist2 + r_diff * weights[0] + start_end_diff * weights[1] + d_diff * weights[2] + twl_avg * weights[3]
            # priority_tier = dist1 + dist2 + (r_diff + d_diff) * weights[0] + start_end_diff * weights[1]
...
def get_init_bundle(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, bundle_merging_function, default_get_dist_method, weights, order_count_upper_limit=3):
    ...
    all_bundles, rider_abailables = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, order_count_upper_limit, 'two_seq', all_bundles, bundle_merging_function, default_get_dist_method, weights)
...
def get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, bundle_merging_function, default_get_dist_function, weights):
    for rider_i in range(3):
        ALL_RIDERS[rider_i].available_number = init_availables[rider_i]
    ...
    # 2개 주문 묶음 생성
    all_bundles, _ = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, 2, 'two_seq', all_bundles, bundle_merging_function, default_get_dist_function, weights)

    # 4개 주문 묶음 생성
    all_bundles, _ = kruskal_bundling(K, DIST, ALL_ORDERS, ALL_RIDERS, 4, 'avg', all_bundles, bundle_merging_function, default_get_dist_function, weights)
...
def check_reassign_riders(K, ALL_ORDERS, ALL_RIDERS, DIST, init_availables, all_bundles):
    ...
    for i in range(len(bundle_info)):
        ...
        for cost, rider_type in costs:
            ...
            if rider_availables[rider_index] > 0:
...
def get_init_bundle_4_order_bundle_prefered_with_reassigning_riders(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, bundle_merging_function, defualt_get_dist_function, weights):
    all_bundles, rider_availables, _ = get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, bundle_merging_function, defualt_get_dist_function, weights)
    for rider_i in range(3):
...
def select_init_bundle_method(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, weights, selected_num):
    if selected_num == 0:
        all_bundles, rider_availables, _ = get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, try_merging_bundles_by_dist_with_car_walk_prefered, get_dist_by_coords, weights)
        for rider_i in range(3):
            ALL_RIDERS[rider_i].available_number = rider_availables[rider_i]
        ...
        return all_bundles, rider_availables, sum((bundle.cost for bundle in all_bundles)) / K
    elif selected_num == 1:
        all_bundles, rider_availables, _ = get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, try_merging_bundles_by_dist, get_dist_by_coords, weights)
        for rider_i in range(3):
            ALL_RIDERS[rider_i].available_number = rider_availables[rider_i]
        ...
        all_bundles, rider_availables, _ = get_init_bundle(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, try_merging_bundles_by_dist_with_car_walk_prefered, get_dist_by_coords, weights, order_count_upper_limit=4)
        ...
        all_bundles, rider_availables, _ = get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, try_merging_bundles_by_dist_with_car_walk_prefered, get_minkowski_dist_by_coords, weights)
        for rider_i in range(3):
            ALL_RIDERS[rider_i].available_number = rider_availables[rider_i]
        ...
        all_bundles, rider_availables, _ = get_init_bundle_4_order_bundle_prefered(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, try_merging_bundles_by_dist, get_minkowski_dist_by_coords, weights)
        for rider_i in range(3):
            ALL_RIDERS[rider_i].available_number = rider_availables[rider_i]
        ...
        all_bundles, rider_availables, _ = get_init_bundle(K, ALL_RIDERS, ALL_ORDERS, DIST, init_availables, try_merging_bundles_by_dist_with_car_walk_prefered, get_minkowski_dist_by_coords, weights, order_count_upper_limit=4)
...
def try_feasible_route_and_rider(K, dist_mat, all_orders, all_riders, orders):
    total_volume = get_total_volume(all_orders, orders)
    ...
    if min_total_dist != inf:
        return Bundle(all_orders, min_total_dist_rider, list(min_total_dist_shop_pen), list(min_total_dist_dlv_pen), total_volume, min_total_dist)
    else:     
        return None

def try_feasible_route(K, dist_mat, all_orders, all_riders, orders, rider):
    total_volume = get_total_volume(all_orders, orders)
    ...
    if min_total_dist != inf:
        return Bundle(all_orders, rider, list(min_total_dist_shop_pen), list(min_total_dist_dlv_pen), total_volume, min_total_dist)
    else:     
        return None
...
# walk, car, bike 순으로 우선하여 확인 후 반환함
def try_merging_bundles_by_dist_with_walk_car_prefered(K, dist_mat, all_orders, all_riders, bundle1, bundle2, order_count_upper_limit=3, order_count_lower_limit=1):
    car_rider = [rider for rider in all_riders if rider.type == 'CAR'][0]
    ...
    for rider in [walk_rider, car_rider, bike_rider]:
        if rider.available_number == 0:
            continue
...
# car, walk, bike 순으로 우선하여 확인 후 반환함
def try_merging_bundles_by_dist_with_car_walk_prefered(K, dist_mat, all_orders, all_riders, bundle1, bundle2, order_count_upper_limit=3, order_count_lower_limit=1):
    car_rider = [rider for rider in all_riders if rider.type == 'CAR'][0]
    bike_rider = [rider for rider in all_riders if rider.type == 'BIKE'][0]
    walk_rider = [rider for rider in all_riders if rider.type == 'WALK'][0]
    ...
    for rider in [car_rider, walk_rider, bike_rider]:
        if rider.available_number == 0:
            continue
